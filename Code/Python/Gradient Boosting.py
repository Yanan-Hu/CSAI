...
param_dist = {'classification__n_estimators': randint(50, 150), 'classification__learning_rate': uniform(0.01, 0.5),
              'classification__max_depth': randint(3, 10),'classification__min_samples_leaf': randint(20, 100),
              'classification__min_samples_split': randint(20, 100), 'classification__subsample': uniform(0.5, 0.5),
...
with open('../results/results.txt', 'a') as a:
    print('Gradient boosting', file=a)
...
# save model
joblib.dump(best_pipeline, f'../model/best_pipeline_gb.clf')

# best classifier
best_classifier = best_pipeline.named_steps['classification']

# save model
joblib.dump(best_classifier, f'../model/best_classifier_gb.clf')

# save csv
csv_data = pd.DataFrame({'Model': ['Gradient boosting'], 'mean test score': mean_test_score, 'SD test score': std_test_score})
