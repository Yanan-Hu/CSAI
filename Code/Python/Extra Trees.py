...
param_dist = {'classification__n_estimators': randint(50, 150), 'classification__max_depth': randint(3, 10),
              'classification__min_samples_leaf': randint(20, 100), 'classification__min_samples_split': randint(20, 100),
...
# save model
joblib.dump(best_pipeline, f'../model/best_pipeline_extratrees.clf')

# best classifier
best_classifier = best_pipeline.named_steps['classification']

# save model
joblib.dump(best_classifier, f'../model/best_classifier_extratrees.clf')

# save csv
csv_data = pd.DataFrame({'Model': ['Extra trees'], 'mean test score': mean_test_score, 'SD test score': std_test_score})
