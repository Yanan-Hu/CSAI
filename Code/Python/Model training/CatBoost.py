...
# creat model
estimator = CatBoostClassifier(random_state=seed, verbose=0, allow_writing_files=False)

# hyperparameter tuning and feature selection
rfecv = RFECV(estimator=estimator, step=1, cv=5, scoring='roc_auc')
param_dist = {'classification__iterations': randint(50, 150), 'classification__learning_rate': uniform(0.01, 0.5),
              'classification__depth': randint(3, 10),'classification__l2_leaf_reg': uniform(1, 10),
              'classification__bagging_temperature': uniform(0, 1)}
...
# save model
joblib.dump(best_pipeline, f'../model/best_pipeline_catboost.clf')

# best classifier
best_classifier = best_pipeline.named_steps['classification']

# save model
joblib.dump(best_classifier, f'../model/best_classifier_catboost.clf')

# save csv
csv_data = pd.DataFrame({'Model': ['Catboost'], 'mean test score': mean_test_score, 'SD test score': std_test_score})
