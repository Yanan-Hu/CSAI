...
param_dist = {'classification__n_estimators': randint(50, 150), 'classification__learning_rate': uniform(0.01, 0.5),
              'classification__algorithm': ['SAMME','SAMMME.R']}
...
# save model
joblib.dump(best_pipeline, '../model/best_pipeline_adaboost.clf')

# best classifier
best_classifier = best_pipeline.named_steps['classification']

# save model
joblib.dump(best_classifier, '../model/best_classifier_adaboost.clf')

# save csv
csv_data = pd.DataFrame({'Model': ['Adaboost'], 'mean test score': mean_test_score, 'SD test score': std_test_score})
