...
# creat model
estimator = XGBClassifier (random_state=seed, verbosity=0)
...
param_dist = {'classification__n_estimators': randint(50, 150), 'classification__learning_rate': uniform(0.01, 0.5),
              'classification__max_depth': randint(3, 10),'classification__subsample': uniform(0.5, 0.5),
              'classification__colsample_bytree': uniform(0.5, 0.5),'classification__gamma': uniform(0, 5)}
...
# save model
joblib.dump(best_pipeline, '../model/best_pipeline_xgboost.clf')

# best classifier
best_classifier = best_pipeline.named_steps['classification']

# save model
joblib.dump(best_classifier, '../model/best_classifier_xgboost.clf')

# save csv
csv_data = pd.DataFrame({'Model': ['XGBoost'], 'mean test score': mean_test_score, 'SD test score': std_test_score})
