...
# creat model
estimator = KNeighborsClassifier(random_state=seed)


# hyperparameter tuning and feature selection
rfecv = RFECV(estimator=estimator, step=1, cv=5, scoring='roc_auc')
param_dist = {'classification__n_neighbors': randint(2, 10),'classification__weights': ['uniform', 'distance'],
              'classification__algorithm':['ball_tree', 'kd_tree'],'classification__leaf_size': randint(30, 100),
...
# save model
joblib.dump(best_pipeline, f'../model/best_pipeline_knn.clf')

# best classifier
best_classifier = best_pipeline.named_steps['classification']

# save model
joblib.dump(best_classifier, f'../model/best_classifier_knn.clf')

# save csv
csv_data = pd.DataFrame({'Model': ['KNN'], 'mean test score': mean_test_score, 'SD test score': std_test_score})
